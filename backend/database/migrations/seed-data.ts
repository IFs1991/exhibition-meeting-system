...
export class SeedData1709123456789 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // テストユーザーの作成
    const hashedPassword = await hash('test1234', 10);
    await queryRunner.query(
      `INSERT INTO users (email, password_hash, name, role) 
       VALUES ($1, $2, $3, $4)`,
      ['test@example.com', hashedPassword, 'テストユーザー', 'PRACTITIONER']
    );
    ...
    for (const sample of sampleCases) {
      const result = await queryRunner.query(
        `INSERT INTO receipt_cases (title, symptoms, treatment, created_by) 
         VALUES ($1, $2, $3, (SELECT id FROM users WHERE email = 'test@example.com')) 
         RETURNING id`,
        [sample.title, sample.symptoms, sample.treatment]
      );
